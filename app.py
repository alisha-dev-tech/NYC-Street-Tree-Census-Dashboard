...
# Light Brown & Sage Green Theme - FIXED
st.markdown("""
<style>
    :root {
        --light-brown: #D4A574;
    ...
    [data-testid="stSidebar"] { background-color: #F5F1ED; }
   .main { background-color: #FAF8; }
    h1, h2, h3 { color: #6B5344; font-weight: 700; }
   ...
   .stButton>button:hover { background-color: #7a9370; }
   ...
   .stSlider>div>div>div { background-color: #9CAF88; }

    /* FIX: Multiselect tags color */
...
def load_data():
    try:
        ...
        st.error("CSV file not found! 'data/2015_Street_Tree_Census_-_Tree_Data.csv' me daal do")
        st.stop()
...
with col1: st.plotly_chart(all_charts['pie'], use_container_width=True)
with col2: st.plotly_chart(all_charts['histogram'], use_container_width=True)
with col3: st.plotly_chart(all_charts['line'], use_container_width=True)

col1, col2, col3 = st.columns(3)
with col1: st.plotly_chart(all_charts['bar'], use_container_width=True)
with col2: st.plotly_chart(all_charts['scatter'], use_container_width=True)
with col3: st.plotly_chart(all_charts['box'], use_container_width=True)

col1, col2, col3 = st.columns(3)
with col1: st.plotly_chart(all_charts['heatmap'], use_container_width=True)
with col2: st.plotly_chart(all_charts['area'], use_container_width=True)
with col3: st.plotly_chart(all_charts['count'], use_container_width=True)

col1, col2, col3 = st.columns([1, 1, 1])
with col2: st.plotly_chart(all_charts['violin'], use_container_width=True)
