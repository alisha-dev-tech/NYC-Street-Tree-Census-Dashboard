...
def load_data():
    ...
    df = pd.read_csv(url, nrows=5000, parse_dates=['created_at'])
    return df
...
if data_loaded and df is not None:
    ...
    def init_state():
        ...
        st.session_state.ss_end_date   = date_max
        st.session_state.ss_boroughs   = boroughs[:]
        st.session_state.ss_health     = health_status[:]
        st.session_state.ss_species    = top_species[:]
        st.session_state.ss_user_type  = user_types[:]
        st.session_state.ss_dbh_range  = default_dbh
    ...
    **Features:**
    ...
    def chart_heatmap(data):
        ...
        return base_layout(fig, "Numeric Feature Correlation Heatmap", height=500)
